...
diffusion.pretrain_unet="Yuyang-z/genxd" \
diffusion.output_dir="outputs/gen3d-custom" \
+evaluator.data_name="static_cam_single" \
+evaluator.data_root="data/custom" \
+evaluator.motion_strength=0.0 +evaluator.indicator=0 \
+evaluator.scale=1.0 +evaluator.mode="first" \
...
+evaluator.motion_strength=0. +evaluator.indicator=0 \
+evaluator.scale=1.0 +evaluator.num_context_views=3 +evaluator.n_views=3 \
