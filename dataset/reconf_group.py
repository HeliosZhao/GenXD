...
## evaluation dataset
class ReconFusionGroupDataset(Dataset):
    '''
    ReconFusion Dataset Generation with Grouping
    '''
    near_far = {
        "re10k": (1, 100),
        "co3d": (0.5, 40),
    ...
    def __init__(self,
        ...
        for key, torchpath in index.items():
            ...
            gen_poses = json.load(open(os.path.join(pose_dir, key, f"gen_poses_w2c.json"), 'r')) # N,3,4
            gen_poses = torch.tensor(gen_poses).reshape(-1, 3, 4) # G,3,4
            cond_c2w_t = pad_pose(condition_extrinsics).inverse()[:,:3, -1] # C,3
            gen_c2w_t = pad_pose(gen_poses).inverse()[:,:3, -1] # G,3
            # ipdb.set_trace()
            if group_strategy == "sequential":
                ## sequential grouping
                if num_pose_per_traj is not None:
                    nearest_indices = torch.arange(num_targets).reshape(1, -1) # 1, num_targets
                    num_groups_per_traj = num_pose_per_traj // num_targets
                    num_traj = gen_poses.size(0) // num_pose_per_traj
                    print(f"totally {gen_poses.size(0)} poses, {num_traj} trajectories, {num_groups_per_traj} groups per trajectory")
                    # num_groups = gen_poses.size(0) // num_targets
                    offset = [torch.arange(num_groups_per_traj) * num_targets + num_pose_per_traj * traj_id for traj_id in range(num_traj)]
                    offset = torch.cat(offset, dim=0).reshape(-1, 1) # num_groups, 1
                    nearest_indices = nearest_indices.repeat(offset.size(0), 1) + offset # num_groups, num_targets
                else:
                    nearest_indices = torch.arange(num_targets).reshape(1, -1) # 1, num_targets
                    num_groups = gen_poses.size(0) // num_targets
                ...
                ## group the target poses
                positions_for_sampling = torch.cat([cond_c2w_t[:1], gen_c2w_t], dim=0).float() # G+1,3
                points, anchor_indices = farthest_point_sample_tensor(
                    positions_for_sampling, 
                    positions_for_sampling.shape[0]//num_targets + 1, 
                    0) # always start from the condition frame
                anchor_indices = anchor_indices
                anchor_positions = points
                distances = torch.norm(gen_c2w_t.unsqueeze(0) - anchor_positions.unsqueeze(1), dim=-1) # N,G
                nearest_indices = torch.argsort(distances, dim=1)[:, :num_targets] # N, num_targets
    ...
    def __getitem__(self, index):
        ...
        context_images = self.convert_images(condition_images) # F,3,H,W
